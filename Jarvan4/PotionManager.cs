...
namespace Jarvan4
{
    class PotionManager
    {
        ...
        private void Load()
        {
            ...
            JMenu.MenuExtras.AddSubMenu(new Menu("Potion Manager", "PotionManager"));

            JMenu.MenuExtras.SubMenu("PotionManager").AddSubMenu(new Menu("Health", "Health"));
            JMenu.MenuExtras.SubMenu("PotionManager")
                .SubMenu("Health")
            ...
            JMenu.MenuExtras.SubMenu("PotionManager").AddSubMenu(new Menu("Mana", "Mana"));
            JMenu.MenuExtras.SubMenu("PotionManager")
                .SubMenu("Mana")
        ...
        private void OnGameUpdate(EventArgs args)
        {
            ...
            try
            {
                if (JMenu.MenuExtras.Item("HealthPotion").GetValue<bool>())
                {
                    if (GetPlayerHealthPercentage() <= JMenu.MenuExtras.Item("HealthPercent").GetValue<Slider>().Value)
                    {
                ...
                if (JMenu.MenuExtras.Item("ManaPotion").GetValue<bool>())
                {
                    if (GetPlayerManaPercentage() <= JMenu.MenuExtras.Item("ManaPercent").GetValue<Slider>().Value)
                    {
